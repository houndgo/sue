...
var nickOneLen = len(nikeOne)
var nikeTwoLen = len(nikeTwo)

func GetFullName(num int64) string {
	rand.Seed(time.Now().UnixNano() * num) //设置随机数种子
	var first string                       //名
...
func GetNikeName(num int64) string {
	rand.Seed(time.Now().UnixNano() * num) //设置随机数种子
	var first string
	first = fmt.Sprint(nikeOne[rand.Intn(nickOneLen-1)])
	//返回昵称
	return fmt.Sprintf("%s%s", first , fmt.Sprint(nikeTwo[rand.Intn(nikeTwoLen-1)]))
}